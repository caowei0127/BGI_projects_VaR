...
def _get_price_var_cov_():
    df_EOD_price = DataFrame(spreadsheet.worksheet(
        property='id', value=EOD_price).get_all_records()).drop(
        ['date'], axis=1)
    df_price_return = np.log(df_EOD_price.copy().pct_change() +
                             1).dropna().astype('float')
...
def _get_implied_var_cov_(df_var_cov, df_price_return, implied_vol):
    ...
    df_implied_vol.drop(['core_symbol', 'exchange_rate', 'vol symbol'], axis=1, inplace=True)
    df_implied_vol.set_index(['bbg symbol'], inplace=True)F
    print('df_implied_vol: \n', df_implied_vol)
    df_implied_vol = df_implied_vol / (np.sqrt(252) * 100)
    print(df_var_cov.join(df_implied_vol).iloc[:, -1:])
    var_cov_implied = np.matmul(df_var_cov.join(df_implied_vol).iloc[:, -1:].values, df_var_cov.join(df_implied_vol).iloc[:, -1:].T.values,
                                out=None) * df_price_return.corr(method='pearson').as_matrix()
...
def _get_avg_dev_(mean, var_cov, df_weightage):
    avg_return = float(
        np.matmul(mean.values, df_weightage.values)[0])
    dev = float(np.matmul(np.matmul(
        df_weightage.T.values, var_cov, out=None), df_weightage.values, out=None))
    std_dev = dev ** 0.5
...
def main(argv=None):
    ...
    for margin_account_number in mc_lp.keys():
        df_weightage, portfolio_value = _get_weightage_(df_var_cov, _get_lp_position_(
        ...
        _get_var_result_(avg_return, avg_return_5, std_dev, std_dev_5, df_var, mc_lp[margin_account_number], portfolio_value, free_equity, margin, 'historical')
